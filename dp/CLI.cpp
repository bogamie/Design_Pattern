...
void CLI::userMenu() {
    ...
    while (true) {
        cout << "\n-- User Menu -- (" << currentUser_->getName()
            << ", " << currentUser_->getGrade() << ", Coupon: " 
            << (currentUser_->getCoupon() ? "Yes" : "No") << ")" << endl;
        cout << "1) View Books" << endl;
        cout << "2) Rent Book" << endl;
        cout << "3) Reserve Book" << endl;
        cout << "4) Return Book" << endl;
        cout << "5) Calculate Rental Fee" << endl;
        cout << "6) Manage Coupon" << endl;
        cout << "0) Back" << endl;
        ...
        case 5: userCalculateFee(); break;
        case 6: userManageCoupon(); break;
        case 0: return;
...
void CLI::userCalculateFee() {
    int id = promptInt("Book ID to calculate fee: ");
    Book* book = sys_.getBook(id);
    if (!book) {
        cout << "Book not found." << endl;
        return;
    ...
    if (!book->rented()) {
        cout << "This book is not currently rented." << endl;
        return;
    }
    
    int days = promptInt("Enter rental days: ");
    book->setRentalDays(days);
    
    double fee = sys_.rental().calculateRentalFee(currentUser_, book);
    
    cout << "\n===== 대여 요금 계산 결과 =====" << endl;
    cout << "책: " << book->getTitle() << endl;
    cout << "대여 일수: " << days << endl;
    cout << "등급: " << currentUser_->getGrade() << endl;
    cout << "쿠폰 사용: " << (currentUser_->getCoupon() ? "예" : "아니오") << endl;
    cout << "최종 요금: " << fee << " 원" << endl;
    cout << "===============================\n" << endl;
}

void CLI::userManageCoupon() {
    cout << "\n-- Coupon Management --" << endl;
    cout << "Current Coupon Status: " << (currentUser_->getCoupon() ? "Has Coupon" : "No Coupon") << endl;
    cout << "1) Get Coupon" << endl;
    cout << "2) Use Coupon" << endl;
    cout << "0) Back" << endl;
    ...
    switch (sel) {
    case 1:
        if (currentUser_->getCoupon()) {
            cout << "You already have a coupon." << endl;
        } else {
            currentUser_->setCoupon(true);
            cout << "Coupon acquired! (500 KRW discount)" << endl;
        }
        break;
    case 2:
        if (!currentUser_->getCoupon()) {
            cout << "You don't have a coupon." << endl;
        } else {
            currentUser_->setCoupon(false);
            cout << "Coupon used." << endl;
        }
        break;
    case 0:
        return;
